...
Modified by Adescom Developers
...
lms-payments.php
(C) 2001-2016 LMS Developers
Modified by Adescom Developers, originating from 1ecdf706bbaad69577020f415d96f635fb89c8bf
...
$query = "SELECT a.tariffid, a.liabilityid, a.customerid, a.recipient_address_id,
		...
		(CASE a.liabilityid WHEN IS NULL THEN t.type ELSE -1 END) AS tarifftype, 
		(CASE a.liabilityid WHEN IS NULL THEN t.name ELSE l.name END) AS name, 
		(CASE a.liabilityid WHEN IS NULL THEN t.taxid ELSE l.taxid END) AS taxid, 
		(CASE a.liabilityid WHEN IS NULL THEN t.prodid ELSE l.prodid END) AS prodid, 
		ROUND(((((100 - a.pdiscount) * (CASE a.liabilityid WHEN IS NULL THEN t.value ELSE l.value END)) / 100) - a.vdiscount) *
...
// Adescom
$adescom_synchronize = ConfigHelper::getConfig('adescom.synchronize', false);
$adescom_add_calls = ConfigHelper::getConfig('adescom.invoice_add_calls', false);
$adescom_separate_fractions = ConfigHelper::getConfig('adescom.invoice_separate_fractions', false);
$adescom_invoice_position_calls = ConfigHelper::getConfig('adescom.invoice_position_calls', 'Połączenie telefoniczne: %calls_fraction (%calls_count szt.) za okres od %calls_periodfrom do %calls_periodto');
$adescom_invoice_position_subscribe = ConfigHelper::getConfig('adescom.invoice_position_subscribe', 'Abonament telefoniczny za miesiąc: %subscribe_period');

echo 'adescom.synchronize: ' . var_export($adescom_synchronize, true) . PHP_EOL;
echo 'adescom.invoice_add_calls: ' . var_export($adescom_add_calls, true) . PHP_EOL;
echo 'adescom.invoice_separate_fractions: ' . var_export($adescom_separate_fractions, true) . PHP_EOL;
echo 'adescom.invoice_position_calls: ' . var_export($adescom_invoice_position_calls, true) . PHP_EOL;
echo 'adescom.invoice_position_subscribe: ' . var_export($adescom_invoice_position_subscribe, true) . PHP_EOL;

function adescom_get_calls_period($period, $dom, $month, $year, $weekday = null)
{
    if (empty($period)) {
        $period = YEARLY;
    }
    ...
    switch ($period) {
        case DAILY:
            $start = strftime("%s", mktime(0, 0, 0, $month, $dom - 1, $year));
            $end = strftime("%s", mktime(23, 59, 59, $month, $dom - 1, $year));
            break;
        case WEEKLY:
            $startweek = $dom - $weekday + 1;
            $start = strftime("%s", mktime(0, 0, 0, $month, $startweek - 7, $year));
            $end = strftime("%s", mktime(23, 59, 59, $month, $startweek - 1, $year));
            break;
        case MONTHLY:
            $start = strftime("%s", mktime(0, 0, 0, $month - 1, 1, $year));
            $end = strftime("%s", mktime(23, 59, 59, $month, 0, $year));
            break;
        case QUARTERLY:
            if ($month <= 3) {
                $startmonth = 1;
            ...
            $start = strftime("%s", mktime(0, 0, 0, $startmonth - 3, 1, $year));
            $end = strftime("%s", mktime(23, 59, 59, $startmonth, 0, $year));
            break;
        case HALFYEARLY:
            if ($month <= 6) {
                $startmonth = 1;
            ...
            $start = strftime("%s", mktime(0, 0, 0, $startmonth - 6, 1, $year));
            $end = strftime("%s", mktime(23, 59, 59, $startmonth, 0, $year));
            break;
        case CONTINUOUS:
            $start = strftime("%s", mktime(0, 0, 0, 1, 1, 1970));
            ...
            break;
        default:
            $start = strftime("%s", mktime(0, 0, 0, 1, 1, $year - 1));
            $end = strftime("%s", mktime(23, 59, 59, 12, 31, $year - 1));
    }
    return array('start' => $start, 'end' => $end);
}

function adescom_getinvoicevalue($customer_id, $date_from, $date_to, SoapClient $connection, $adescom_separate_fractions)
{
    $options = new stdClass();
    $options->incoming = 0;
    $options->outgoing = 1;
    $options->includeZeroDuration = 0;

    $stamp_to = $date_to;

    $date_from_str = date('Y/m/d H:i:s', $date_from);
    $date_to_str = date('Y/m/d H:i:s', $date_to);
    
    echo sprintf("customer_id: %d, from: '%s', to '%s'\n", $customer_id, $date_from_str, $date_to_str);

    $response = $connection->getBillingForClientByExternalID(
        $customer_id, $date_from_str, $date_to_str, $options
    );
    ...
    if ($response->totalCount !== 0) {
        foreach ($response->items as $item) {
            if (!isset($item->fraction) || empty($item->fraction) && $item->destination !== 'CLIENT_SUBSCRIBE') {
                continue;
            }

            $fraction = $item->fraction;
            $price = $item->priceInclTaxes;
            $period = $item->startDate;

            if ($adescom_separate_fractions) {
                $id = $item->id;
                $dst = $item->destination;

                if ($dst === 'CLIENT_SUBSCRIBE') {
                    $fraction = 'SUBSCRIBE_' . $id;
            ...
            if (!array_key_exists($fraction, $output)) {
                $output[$fraction] = array(
                    'fraction' => $fraction,
                    'cost' => 0, 'count' => 0,
                    'period' => $period,
                    'subscribe' => $dst === 'CLIENT_SUBSCRIBE'
                );
            }
            $output[$fraction]['cost'] += $price;
            $output[$fraction]['count'] ++;
        }
    }
...
$adescom_connection = null;
if ($adescom_synchronize) {
    $adescom_connection = AdescomConnection::getConnection();
}

//echo 'before: ' . count($assigns) . PHP_EOL;
//echo 'before: ' . var_export($assigns, true) . PHP_EOL;

$def_taxid = $DB->GetOne('SELECT id FROM taxes WHERE value = ?', array(23));
if (empty($def_taxid)) {
    $DB->Execute('INSERT INTO taxes (value, label) VALUES (?, ?)', array(23, 'Adescom'));
    $def_taxid = $DB->GetLastInsertID('taxes');
}

if ($adescom_connection) {
    foreach ($assigns as $aa => $assign) {
        if ($assign['name'] === 'ADESCOM_AUTO_VOIP_CALL') {
            if ($adescom_add_calls && $adescom_invoice_position_calls) {
                $period = adescom_get_calls_period($assign['period'], $dom, $month, $year, $weekday);
                $calls_periodfrom = date("d/m/Y", $period['start']);
                $calls_periodto = date("d/m/Y", $period['end']);

                try {
                    $response = adescom_getinvoicevalue($assign['customerid'], $period['start'], $period['end'], $adescom_connection, $adescom_separate_fractions);

                    foreach ($response as $value) {
                        
                        $new_assign = $assign;
                        
                        $name = $adescom_invoice_position_calls;
                        $price = $value['cost'];
                        $count = $value['count'];
                        ...
                        if ($adescom_separate_fractions) {
                            ...
                            if ($subscribe) {
                                $name = $adescom_invoice_position_subscribe;
                                $subscribe_period = date("m/Y", $value['period']);
                                $name = preg_replace('/%subscribe_period/', $subscribe_period, $name);
                            } else {
                                $name = preg_replace('/%calls_fraction/', $fraction, $name);
                            }
                        }

                        $name = preg_replace('/%calls_count/', $count, $name);
                        $name = preg_replace('/%calls_periodfrom/', $calls_periodfrom, $name);
                        $name = preg_replace('/%calls_periodto/', $calls_periodto, $name);

                        $new_assign['name'] = $name;
                        $new_assign['value'] = $price;
                        $new_assign['taxid'] = $def_taxid;

                        array_push($assigns, $new_assign);
                    }
                    
                } catch (Exception $ex) {
                    
                    echo 'customerid: ' . $assign['customerid'] . PHP_EOL;
...
//echo 'after: ' . count($assigns) . PHP_EOL;
//echo 'after: ' . var_export($assigns, true) . PHP_EOL;
